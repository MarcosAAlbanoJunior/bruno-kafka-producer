...
/**
 * @param {object} ctx  { topic?, environmentName?, app?, isProduction?, connection, schemaRegistry? }
 * @returns {Promise<{checks:object[], ok:boolean}>}
 */
async function runDiagnostics(ctx) {
  const { topic, environmentName, app, isProduction, connection, schemaRegistry } = ctx;
  const checks = [];
  ...
  try {
    ...
    require.resolve('jks-js');
    require.resolve('@kafkajs/confluent-schema-registry');
    add('Dependencias npm', OK, 'kafkajs, jks-js e confluent-schema-registry instalados');
  } catch (err) {
    add('Dependencias npm', FAIL, err.message, 'Rode "npm install" na pasta da collection.');
