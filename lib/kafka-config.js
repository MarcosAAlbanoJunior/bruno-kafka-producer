...
function createReader(bru) {
  // As variaveis declaradas em "Vars > Pre Request" do request/pasta vivem num
  // escopo PROPRIO (request-scoped). Nas versoes mais novas do Bruno elas so
  // aparecem em bru.getRequestVar(); nas mais antigas, em bru.getVar(). Lemos as
  // duas, senao um kafkaTopic preenchido na tela chega vazio no script.
  const layers = [
    (name) => (typeof bru.getRequestVar === 'function' ? bru.getRequestVar(name) : undefined),
    (name) => (typeof bru.getVar === 'function' ? bru.getVar(name) : undefined),
    (name) => (typeof bru.getEnvVar === 'function' ? bru.getEnvVar(name) : undefined),
    (name) => process.env[toEnvKey(name)],
  ];
  ...
  const lookup = (names) => {
    for (const layer of layers) {
      for (const name of names) {
        let value;
        try { value = layer(name); } catch (err) { value = undefined; }
  ...
  const app = lookup(['kafkaApp']);

  const readOne = (name) => lookup(app ? [`${name}_${app}`, name] : [name]);
